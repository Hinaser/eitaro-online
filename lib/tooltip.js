 ...
 * @param {Function} configer - A function to show configuration panel.
 * @constructor
 */
const Tooltip = function(sanitizer, searcher, configer){
 ...
 * @param db - Instance of require('lib/db')
 * @throws {Error} - When `this.prefs` is empty, throw an Error.
 */
Tooltip.prototype.init = function(prefs, db){
    ...
    // When db is empty, throw an exception.
    if(!db){
        throw new Error("Db instance variable must be set");
    }
    this.db = db;
...
Tooltip.prototype.attach = function (tab=null){
    ...
    worker.port.on("search", (search_keyword) => {
        this.search(search_keyword);
    });

    worker.port.on("config", () => {
        this.config();
    });

    worker.port.on("requestURL", (url) => {
        this.prepare();

        const xhr = Request({
            url: url,
            onComplete: (response) => {
                // Assume that url is invalid or remote server is down.
                if(response.status != 200){
                    this.show("リンク先のページの取得ができませんでした");
                    return;
                }
                ...
                try{
                    ...
                    if(parsedURL){
                        const trimmedHtml = Util.parseSearchResult(response.text, this.prefs.get("service_selector"), this.prefs.get("service_deselector"));

                        if(this.prefs.get("save_search_history")){
                            this.db.add(decodeURIComponent(parsedURL), trimmedHtml);
                        }
                        this.show(trimmedHtml);
                    }
                    else{
                        this.show(response.text)
                    }
                }
                catch(e){
                    this.show(e.message);
...
/**
 * Check that url is configured as search engine. If it is, return "search word" within the url.
 * If it is not, return false.
 * @param {string} url - Url to be checked.
 * @returns {string|boolean} - If the url is not configured as search engine, returns false. Else it returns "search keyword".
 * @throws - If prefs instance is not set yet, throw an Error.
 */
Tooltip.prototype.parseUrl = function(url){
    if(!this.prefs){
        throw new Error("Prefs must be set.");
    }

    const search_engine_url = this.prefs.get("service_url");
    const regex = new RegExp("^" + search_engine_url.replace("{0}", "(.+)") + "$");
    const regex_info = regex.exec(url);
