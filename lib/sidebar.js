...
// let `sidebar` will be a value by `require('sdk/ui/sidebar').Sidebar({...})`
let sidebar = null;
...
let sidebar_content_default_option = {
 ...
 */
const sidebar_id = "eitaro-sidebar";
 ...
 * @constructor
 */
let Sidebar = function(title, db, sanitizer){
    this.id = sidebar_id;
    this.title = title;
    this.db = db;
...
Sidebar.prototype.build_sidebar = function (){
    // In few cases, build_sidebar fails with message like `a combination of title and url is already used`
    // I guess sidebar.dispose() does not work properly sometimes.
    // I attempt to reset sidebar instance here to resolve this problem.
    if(sidebar != null){
        try{
            sidebar.dispose();
            sidebar = null;
    ...
    sidebar = require('sdk/ui/sidebar').Sidebar({
        ...
        onDetach: function(worker){
            let index = sidebar_workers.indexOf(worker);
            if(index != -1){
...
Sidebar.prototype.showSearchResult = function(result, option){
    ...
    // Set font size
    sidebar_content.option.fontSize = this.fontSize;
...
/**
 * Request sidebar content script to change font size.
 * @param fontSize
 */
Sidebar.prototype.setFontSize = function (fontSize) {
    this.fontSize = fontSize;

    // if sidebar is expaneded
    if(!(sidebar == null || sidebar_workers == null || sidebar_workers.length < 1)){
        sidebar_workers[sidebar_workers.length - 1].port.emit("set_font_size", this.fontSize.toString());
    }
};
...
Sidebar.prototype.sanitize_sidebar_content = function (){
    ...
    if(sidebar_content.type == 'history'){
        if(sidebar_content.history_data && sidebar_content.history_data.records){
            for(let i=0;i<sidebar_content.history_data.records.length;i++){
                // ** SANITIZING ** //
