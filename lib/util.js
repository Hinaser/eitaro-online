 ...
 */
const trim_space = function (word) {
 ...
 */
const sanitizeHtml = function (html_text) {
    let document = parser.parseFromString(html_text, "text/html");
    ...
    let html_flagment = parseHTML(document, html_text, true, null, false);
 ...
 * @returns {string}
 */
const parseSearchResult = function (html_as_string, selector, deselector){
    let content_to_return = null;
    let document = parser.parseFromString(html_as_string, "text/html");
 ...
 */
const exportDumpToFile = function (db){
    ...
    if(res != nsIFilePicker.returnCancel){
        ...
        db.getAll("dtime", "prev", function(item){
            let TextWriter = fileIO.open(theFile.path, "w");
 ...
 * @param db - Instance of database defined in lib/db.js.
 */
const exportFormattedDataToFile = function (db){
    const nsIFilePicker = Ci.nsIFilePicker;
    let fp = Cc["@mozilla.org/filepicker;1"].createInstance(nsIFilePicker);
    fp.init(require("sdk/window/utils").getMostRecentBrowserWindow().content, messages.select_filename_to_save, nsIFilePicker.modeSave);
